...
def index():
    ...
    stats = {
        ...
        'most_categories' : top_category.iloc[0].reset_index().columns[1],
    ...
    cat_order = df2.groupby('Category').agg({'App' : 'count' }).\
        rename({'App':'Total'}, axis=1).sort_values('Total', ascending = False).head().reset_index()
    ...
    paid_app = playstore[playstore['Type'] == 'Paid']
    paid_app_graph = paid_app.groupby('Category').agg({'Price' : 'mean'}).sort_values(by = 'Price', ascending = False).head(5).rename({'Price':'AVG_Price'}, axis=1).sort_values('AVG_Price').head().reset_index()
    X = paid_app_graph['Category']
    Y = paid_app_graph['AVG_Price']
    my_colors = 'rgbkymc'
    fig = plt.figure(figsize=(7,6))
    fig.add_subplot()
    plt.barh(X,Y, color=my_colors)
    plt.savefig('paid_app.png',bbox_inches="tight")
    figfile = BytesIO()
    plt.savefig(figfile, format='png')
    figfile.seek(0)
    figdata_png = base64.b64encode(figfile.getvalue())
